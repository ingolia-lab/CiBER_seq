...
mpralmpath <- "/mnt/ingolialab/rmuller1/test_sandbox/gz_to_mpralm/rehearsal_dinner/trial1_all_raw_fasta_gz/"
...
pdf("his4_pool_volc_aa_trnasyn.pdf", useDingbats=FALSE, width=5, height=5, colormodel="rgb")
...
plot(x = his4_pooled$LFC_plot, y = his4_pooled$sig_plot,
     ...
     col="gray52")
points(-aabiosynthesis_handcurhis4_pooled$logFC,
       -log10(pmax(10^-20, aabiosynthesis_handcurhis4_pooled$adj.P.Val)),
       type="n",
       col="violet"
)
...
points(-translationcontrol_handcurhis4_pooled$logFC,
       -log10(pmax(10^-20, translationcontrol_handcurhis4_pooled$adj.P.Val)),
       type="n",
       col="royalblue1"
)
...
pointsfile <- "his4_pool_volc_aa_trnasyn.png"
...
pdf("his4_pool_volc_pol_trnaproc.pdf", useDingbats=FALSE, width=5, height=5, colormodel="rgb")
## Plot with type="n" to skip drawing the points
plot(x = his4_pooled$LFC_plot, y = his4_pooled$sig_plot,
     type="n",
     xlim=c(-6, 6), ylim=c(0, 20),
...
points(-tRNAprocessing_handcurhis4_pooled$logFC,
       -log10(pmax(10^-20, tRNAprocessing_handcurhis4_pooled$adj.P.Val)),
       type="n",
       col="paleturquoise"
)
#rect(-6,0,6,-log10(0.05), col = alpha("gray52", 0.3))
...
height <- max(gy) - min(gy)
## Plot points to a PNG file
pointsfile <- "his4_pool_volc_pol_trnaproc.png"
png(pointsfile, width=width, height=height, units="in", res=1500, bg="transparent")
par(mar=c(0,0,0,0))
## Exactly the same plot as above, but no type="n" and instead specify points, color, etc.
plot(x = his4_pooled$LFC_plot, y = his4_pooled$sig_plot,
...
temp2 <- pmin(6, temp1)
utr$LFC_plot <- -temp2
utr$sig_plot <- -log10(pmax(10^-6, utr$adj.P.Val))

pdf("utr_volc_aa_trnasyn.pdf", useDingbats=FALSE, width=5, height=5, colormodel="rgb")
## Plot with type="n" to skip drawing the points
plot(x = utr$LFC_plot, y = utr$sig_plot,
     ...
     ylab = expression('Statistical Significance -log10(Q)'), 
     col="gray52")
points(-aabiosynthesis_handcurutr$logFC,
       -log10(pmax(10^-6, aabiosynthesis_handcurutr$adj.P.Val)),
       type="n",
       col="violet"
)
points(-tRNAaasynthetase_handcurutr$logFC,
       -log10(pmax(10^-6, tRNAaasynthetase_handcurutr$adj.P.Val)),
       type="n",
       col="seagreen4"
)
points(-translationcontrol_handcurutr$logFC,
       -log10(pmax(10^-6, translationcontrol_handcurutr$adj.P.Val)),
       type="n",
       col="royalblue1"
)
#rect(-6,0,6,-log10(0.05), col = alpha("gray52", 0.3))
...
## Plot points to a PNG file
pointsfile <- "utr_volc_aa_trnasyn.png"
png(pointsfile, width=width, height=height, units="in", res=1500, bg="transparent")
par(mar=c(0,0,0,0))
## Exactly the same plot as above, but no type="n" and instead specify points, color, etc.
plot(x = utr$LFC_plot, y = utr$sig_plot,
...
## Draw axes, title, etc.
axis(side = 1, at = c(-6, -4, -2, 0, 2, 4, 6), labels=c("<1/64", "1/16", "1/4", "1", "4", "16", ">64"))
axis(side = 2, at = c(0, 1, 2, 3, 4, 5, 6), labels=c("0", "1", "2", "3", "4", "5", ">6"))
title(main="GCN4 utr Guide Induction")
dev.off()

#utr_ new hits in pol3 and tRNA processing
...
pdf("utr_volc_pol_trnaproc.pdf", useDingbats=FALSE, width=5, height=5, colormodel="rgb")
## Plot with type="n" to skip drawing the points
plot(x = utr$LFC_plot, y = utr$sig_plot,
     type="n",
     xlim=c(-6, 6), ylim=c(0, 6),
     ...
     ylab = expression('Statistical Significance -log10(Q)'),
     col="gray52")
points(-polIIIsubunit_handcurutr$logFC,
       -log10(pmax(10^-6, polIIIsubunit_handcurutr$adj.P.Val)),
       type="n",
       col="palegreen3"
)
points(-tRNAprocessing_handcurutr$logFC,
       -log10(pmax(10^-6, tRNAprocessing_handcurutr$adj.P.Val)),
       type="n",
       col="paleturquoise"
)
#rect(-6,0,6,-log10(0.05), col = alpha("gray52", 0.3))
...
## Plot points to a PNG file
pointsfile <- "utr_volc_pol_trnaproc.png"
png(pointsfile, width=width, height=height, units="in", res=1500, bg="transparent")
par(mar=c(0,0,0,0))
## Exactly the same plot as above, but no type="n" and instead specify points, color, etc.
plot(x = utr$LFC_plot, y = utr$sig_plot,
